...
int sqlstatement::prepare(const char *fmt, ...) {
  ...
  for (int ii = 0; ii < ilen; ii++) {
	if ((m_sql[ii] == ':') && (isdigit(m_sql[ii + 1]) != 0)) {
	  m_sql[ii] = '?';
	  m_sql[ii + 1] = ' ';
	  if (isdigit(m_sql[ii + 2]) != 0)
		m_sql[ii + 2] = ' ';
	  if (isdigit(m_sql[ii + 3]) != 0)
		m_sql[ii + 3] = ' ';
	}
  }
  ...
  if (strstr(m_sql, "str_to_date") == 0)
	MY__UpdateStr(m_sql, "to_date", "str_to_date", false);
...
int sqlstatement::bindin(unsigned int position, int *value) {
  if ((position < 1) || (position >= MAXPARAMS) || (position > m_handle->param_count)) {
	m_cda.rc = -1;
	strncpy(m_cda.message, "array bound.", 128);
  }
