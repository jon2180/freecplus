...
int connection::connecttodb(char *connstr, char *charset, unsigned int autocommitopt) {
  ...
  if (PQstatus(m_conn) != CONNECTION_OK) {
	m_cda.rc = -1;
	strcpy(m_cda.message, PQerrorMessage(m_conn));
	return -1;
  }
...
int sqlstatement::bindin(unsigned int position, char *value, unsigned int len) {
  ...
  if (len > MAXFIELDLENGTH)
	m_paramLengthsIn[m_nParamsIn] = MAXFIELDLENGTH;
...
int sqlstatement::execute() {
  ...
  */
  ...
  if (m_sqltype == 1) {
	if (PQresultStatus(m_res) != PGRES_COMMAND_OK) {
	  m_cda.rc = -1;
	  strcpy(m_cda.message, PQerrorMessage(m_conn->m_conn));
...
int sqlstatement::next() {
  ...
  if (m_sqltype != 0) {
	m_cda.rc = -1;
	strcpy(m_cda.message, "no recordset found.\n");
	return -1;
  }

  if (m_cda.rpc >= (unsigned long)PQntuples(m_res)) {
	m_cda.rc = 1403;
	strcpy(m_cda.message, "no data found");
	return m_cda.rc;
  }
