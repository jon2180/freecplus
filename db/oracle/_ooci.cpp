...
void connection::err_report() {
  ...
  if (m_cxt.errhp != NULL) {
	if (OCIErrorGet(m_cxt.errhp, 1, NULL, &m_cda.rc, (OraText *)m_cda.message, sizeof(m_cda.message), OCI_HTYPE_ERROR)
		== OCI_NO_DATA) {
	  // 如果获取不到错误信息，就返回正确的
	  memset(&m_cda, 0, sizeof(m_cda));
	  return;
...
int sqlstatement::prepare(const char *fmt, ...) {
  ...
  int oci_ret =
	  OCIStmtPrepare(m_handle.smthp, m_handle.errhp, (OraText *)m_sql, strlen(m_sql), OCI_NTV_SYNTAX, OCI_DEFAULT);
...
int sqlstatement::execute() {
  ...
  if (m_sqltype == 1) {
	OCIAttrGet((CONST
	dvoid *)m_handle.smthp, OCI_HTYPE_STMT, (dvoid * ) & m_cda.rpc, (ub4 *)0,
...
void sqlstatement::err_report() {
  ...
  if (m_handle.errhp != NULL) {
	if (OCIErrorGet(m_handle.errhp,
					1,
					NULL,
					&m_cda.rc,
					(OraText *)m_cda.message,
					sizeof(m_cda.message),
